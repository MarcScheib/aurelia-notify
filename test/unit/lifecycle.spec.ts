...
describe('invokeLifecycle', () => {
  ...
  describe('resolves', () => {
    ...
    async function testResult(done: DoneFn, output: any, expected: any = DEFAULT_LIFECYCLE_RESULT, methodName: 'canActivate' | 'canDeactivate' | 'deactivate' = TEST_METHOD_NAME): Promise<void> {
      let result: any;
      vm.output = output;
      try {
  ...
  describe('propagates errors when the method', () => {
    ...
    it('throws', done => {
      ...
      spyOn(vm, TEST_METHOD_NAME).and.callFake(() => {
        throw expectedError;
      });
